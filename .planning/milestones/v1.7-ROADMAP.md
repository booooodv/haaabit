...
v1.7 replaced the previous OpenClaw `skill -> mcporter -> MCP -> API` chain with a native OpenClaw plugin package that talks directly to the shipped Haaabit API. The milestone kept the Haaabit domain model, bearer-token auth contract, and shared client/types single-sourced while removing transport indirection for the host that most needed native tool availability.
...
- Decision: OpenClaw should expose Haaabit through a native plugin instead of another MCP bridge. Outcome: ✓ Good
...
- OpenClaw no longer depends on the `skill -> mcporter -> MCP -> API` chain to reach Haaabit tools.
